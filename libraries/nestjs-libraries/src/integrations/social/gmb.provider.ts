...
export class GmbProvider extends SocialAbstract implements SocialProvider {
  ...
  async pages(accessToken: string) {
    ...
    do {
      ...
      if (accountNextPageToken) {
        queryParams.append('pageToken', accountNextPageToken);
      }

      const accountsResponse = await fetch(
        `https://mybusinessaccountmanagement.googleapis.com/v1/accounts?${queryParams.toString()}`,
        {
          headers: {
      ...
      accountNextPageToken = accountsData.nextPageToken;
    ...
    for (const account of allAccounts) {
      ...
      let locationNextPageToken: string | undefined = undefined;
      ...
      try {
        do {
          ...
          if (locationNextPageToken) {
            queryParams.append('pageToken', locationNextPageToken);
          }

          const locationsResponse = await fetch(
            `https://mybusinessbusinessinformation.googleapis.com/v1/${accountName}/locations?${queryParams.toString()}`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
              },
          ...
          locationNextPageToken = locationsData.nextPageToken;
    ...
    try {
      ...
      const response = await fetch(
        `https://businessprofileperformance.googleapis.com/v1/${locationPath}:fetchMultiDailyMetricsTimeSeries?dailyMetrics=WEBSITE_CLICKS&dailyMetrics=CALL_CLICKS&dailyMetrics=BUSINESS_DIRECTION_REQUESTS&dailyMetrics=BUSINESS_IMPRESSIONS_DESKTOP_MAPS&dailyMetrics=BUSINESS_IMPRESSIONS_MOBILE_MAPS&dailyRange.startDate.year=${dayjs(
          startDate
        ).year()}&dailyRange.startDate.month=${dayjs(startDate).month() + 1
        }&dailyRange.startDate.day=${dayjs(
          startDate
        ).date()}&dailyRange.endDate.year=${dayjs(
          endDate
        ).year()}&dailyRange.endDate.month=${dayjs(endDate).month() + 1
